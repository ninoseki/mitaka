...
import { LocalOptionsSchema, OptionsSchema, type OptionsType, SyncOptionsSchema } from '~/schemas'
...
export async function getOptions(): Promise<OptionsType> {
  ...
  const [syncOptions, localOptions] = await Promise.all([getSyncOptions, getLocalOptions])
