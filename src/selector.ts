...
export class Selector {
  ...
  public constructor(input: string, options: OptionsType = defaultOptions) {
    ...
    this.scanners = Scanners.filter((s) => !this.options.disabledScannerNames.includes(s.name))
  }
  ...
  public getSearchersByType(type: SearchableType): Searcher[] {
    return this.searchers.filter((searcher: Searcher) => searcher.supportedTypes.includes(type))
  }

  public getScannersByType(type: ScannableType): Scanner[] {
    return this.scanners.filter((scanner: Scanner) => scanner.supportedTypes.includes(type))
  }
